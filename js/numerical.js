class Numerical {
            ...
            'newton 2nd law of motion': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.known[ 'force' ] = {
                        value: Numerical.evaluate(dataObject.known.mass.value + '*' + dataObject.known.acceleration.value),
                        unit: Numerical.Data.force.SIunit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'force' });
            ...
            'vector magnitude formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.known[ 'vector magnitude' ] = {
                        value: Numerical.evaluate(`sqrt((${vectorXComponent})^(2) + (${vectorYComponent})^(2))`),
                        unit: Numerical.Data[ 'vector magnitude' ].SIunit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'vector magnitude' });
                    return [ html, dataObject ];
                },
            },
            ...
            'vector magnitude formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.known[ 'vector x-component' ] = {
                        value: Numerical.evaluate(`sqrt((${vectorMagnitude})^(2) - (${vectorYComponent})^(2))`),
                        unit: Numerical.Data[ 'vector x-component' ].SIunit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'vector x-component' });
                    return [ html, dataObject ];
                },
            },
            ...
            'vector magnitude formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.known[ 'vector y-component' ] = {
                        value: Numerical.evaluate(`sqrt((${vectorMagnitude})^(2) - (${vectorXComponent})^(2))`),
                        unit: Numerical.Data[ 'vector y-component' ].SIunit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'vector y-component' });
                    return [ html, dataObject ];
                },
            },
            'vector direction formula': {
        ...
        angle:{
            'vector direction formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.known[ 'angle' ] = {
                        value: Numerical.evaluate(`tan^(-1)(${vectorYComponent}/${vectorXComponent})`),
                        unit: unit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'angle' });
                    return [ html, dataObject ];
                },
            },
            'vector x-component formula': {
                ...
                solve: function (dataObject) {
                    ...
                    let html = Numerical.getHTML(steps);
                    dataObject.known[ 'angle' ] = {
                        value: Numerical.evaluate(`cos^(-1)(${vectorXComponent}/${vectorMagnitude})`),
                        unit: unit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'angle' });
                    return [ html, dataObject ];
            ...
            'vector y-component formula': {
                ...
                solve: function (dataObject) {
                    ...
                    let html = Numerical.getHTML(steps);
                    dataObject.known[ 'angle' ] = {
                        value: Numerical.evaluate(`cos^(-1)(${vectorYComponent} / ${vectorMagnitude})`),
                        unit: unit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'angle' });
                    return [ html, dataObject ];
        ...
        velocity: {
            'velocity formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'velocity' });
                    return [ html, dataObject ];
                },
            },
            ...
            'acceleration formula putting velocity': {
                ...
                solve: function (dataObject) {
                    ...
                    let html = Numerical.getHTML(steps);
                    dataObject.known[ 'velocity' ] = {
                        value: Numerical.evaluate(`sqrt(${acceleration} xx ${displacement})`),
                        unit: unit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'velocity' });
                    return [ html, dataObject ];
        ...
        displacement: { 
            'velocity formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'displacement' });
                    return [ html, dataObject ];
                },
            },
            'acceleration formula putting velocity': {
            ...
            'acceleration formula putting time': {
                ...
                solve: function (dataObject) {
                    let velocity = dataObject.known[ 'velocity' ].value;
                    let velocity = dataObject.known[ 'acceleration' ].value;
                    let unit = Numerical.Data[ 'displacement' ].SIunit;
                    let steps = [
                        `a = v / t`,
                        `\\p\\u\\t\\t\\i\\n\\g\\ t = d / v`,
        ...
        time: {
            'velocity formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'time' });
                    return [ html, dataObject ];
                },
            },
            'acceleration formula': {
            ...
            'acceleration formula putting velocity': {
                ...
                solve: function (dataObject) {
                    ...
                    let html = Numerical.getHTML(steps);
                    dataObject.known[ 'time' ] = {
                        value: Numerical.evaluate(`sqrt(${displacement} / (${acceleration}))`),
                        unit: unit
                    }
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'time' });
                    return [ html, dataObject ];
        ...
        acceleration: {
            'acceleration formula': {
                ...
                solve: function (dataObject) {
                    ...
                    dataObject.unknown = dataObject.unknown.filter(function (el) { return el != 'acceleration' });
                    return [ html, dataObject ];
                },
            },
            'acceleration formula putting velocity': {
    ...
    static getData(dataObject) {
        ...
        let html = '<li data-role="list-divider"><p class="text-center"><strong>`Data`</strong></p></li>'
        for (let data in knownData) {
            if (knownData[ data ].unit == Numerical.Data[ data ].SIunit) {
                ...
                let expr = `${knownData[ data ].value} * ${Numerical.Units[ data ][ knownData[ data ].unit ]}`;
                html += `<li><p><strong>\` ${Numerical.Data[ data ].symbol} = ${Numerical.Data[ data ].mathHtml} = ${knownData[ data ].value} ${knownData[ data ].unit} = ${this.evaluate(expr)} ${Numerical.Data[ data ].SIunit}\`</strong></p></li>`
