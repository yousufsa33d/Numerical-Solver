$(document).on("pagecreate", "#mainpage", function () {
    ...
    solveBtn.addEventListener('click', function () {
        if (data.querySelectorAll('.data').length == 0) {
            errorDetail.innerHTML = 'There is no data to solve. Please check and try again.'
            $('#errorPopup').popup('open');
        }
        else if (data.querySelectorAll('.data[data-value="?"]').length == 0) {
            errorDetail.innerHTML = 'There is no unknown data to solve. Please check and try again.';
            $('#errorPopup').popup('open');
        }
        else {
            let dataObject = {
            ...
            let html = getData[ 0 ];
            html += '<li data-role="list-divider"><p class="text-center"><strong>`Solution`</strong></p></li>'
            ...
            if (formula == 'noFormula') {
                errorDetail.innerHTML = 'There is no formula according to your data stay tune for update.';
                $('#errorPopup').popup('open');
            }
            else {
            ...
            if (formulas == undefined) {
                ...
                for (let formula in formulas) {
                    for (let dataKnown of formulas[ formula ].data) {
                        ...
                        if (formulas[ formula ].data.indexOf(dataKnown) == formulas[ formula ].data.length - 1) {
